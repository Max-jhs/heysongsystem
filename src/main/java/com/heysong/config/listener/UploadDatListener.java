package com.heysong.config.listener;
 ...
 **/
@Slf4j
public class UploadDatListener implements ReadListener<TClue> {
     ...
     * @param tClueMapper
     */
    public UploadDatListener(TClueMapper tClueMapper, String token) {
    ...
    public void invoke(TClue tClue, AnalysisContext context) {
        log.info("解析到一条数据:{}", JSONUtils.beanToJson(tClue));
    ...
    private void saveData() {
        ...
        tClueMapper.saveBatch(cachedDataList);
        log.info("存储数据库成功！");
    }
}